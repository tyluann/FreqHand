...
class RenderLayer(nn.Module):
    ...
    def forward(self, mesh, cam_param, img_affine_trans_mat, mesh):
        ...
        mesh_2d_norm = torch.cat([mesh_2d[:,:,0:1]/cfg.cfg.rendered_img_shape[1]*2-1,\
                (cfg.cfg.rendered_img_shape[0] - 1 - mesh_2d[:,:,1:2])/cfg.cfg.rendered_img_shape[0]*2-1, \
                mesh_z[:,:,None]],2)
        ...
        rendered_depthmap = nr.rasterize_depth(face_vertices, cfg.cfg.rendered_img_shape[0], False, near=cfg.cfg.depth_min, far=cfg.cfg.depth_max)[:,None,:,:]
        rendered_depthmap[rendered_depthmap == cfg.cfg.depth_max] = 0
