...
class GraphConvolution(nn.Module):
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.normal_(self.weight, 0, 0.001)
        torch.nn.init.normal_(self.bias, 0, 0.001)
    ...
    def forward(self, x):
        if x.ndimension() == 2:
            ...
            for i in range(support.shape[0]):
                output.append(torch.matmul(self.adjmat, support[i]))
...
class GraphResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, A):
        ...
        self.pre_norm = nn.GroupNorm((in_channels + 3) // 8, in_channels)
