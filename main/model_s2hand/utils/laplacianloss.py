...
class LaplacianLoss(object):
    ...
    def __call__(self, verts):
        ...
        Lx0 = self.laplacian.apply(verts, self.F_np, self.F, self.L, self.vertices)
        # Reshape to BV x 3
...
class Laplacian(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, V, F_np, F, L, vertices):

        V_np = V.cpu().numpy()
        if F.shape[0] != V_np.shape[0]:
        ...
        if L is None:
            ...
            # Compute cotangents
            verticess = vertices.to(device=V.get_device())
            ...
            if F.dim() == 2:
                ...
                if F.shape[0] != V.shape[0]:
                    F = F[0].unsqueeze(0).repeat(V.shape[0], 1, 1)
            #import pdb; pdb.set_trace()
            #self.F = self.F.to(device=sphere_batchV.device)
            C = cotangent(sphere_batchV, F)
