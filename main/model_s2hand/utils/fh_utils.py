...
def dhm2Frei(dhm_joints):
    #import pdb; pdb.set_trace()
    FreiHand_joints = torch.zeros(dhm_joints.shape[0], dhm_joints.shape[1] - 1, dhm_joints.shape[2]).to(dhm_joints.device) # init empty list

    # dhmId, FreiId
    mapping = {0 : 4,   1 : 3,   2 : 2,   3 : 1,  # Thumb
               5 : 8,   6 : 7,   7 : 6,   8 : 5,  # Index
               9 :12,   10:11,   11:10,   12: 9,  # Middle
               13:16,   14:15,   15:14,   16:13,  # Ring
               17:20,   18:19,   19:18,   20:17,  # Pinky
               21: 0}                             # Wrist

    for dhmId, FreiId in mapping.items():
        FreiHand_joints[:,FreiId] = dhm_joints[:,dhmId]
    #import pdb; pdb.set_trace()
    return FreiHand_joints

def dhm2ManoS2HAND(dhm_joints):
    #import pdb; pdb.set_trace()
    mano_joints = np.zeros(dhm_joints[:21].shape, dtype=np.float32) # init empty list

    # dhmId, FreiId
    mapping = {0 :20,   1 :19,   2 :18,   3 :17,  # Thumb
               5 : 4,   6 : 3,   7 : 2,   8 : 1,  # Index
               9 : 8,   10: 7,   11: 6,   12: 5,  # Middle
               13:16,   14:15,   15:14,   16:13,  # Ring
               17:12,   18:11,   19:10,   20: 9,  # Pinky
               21: 0}                             # Wrist

    for dhmId, manoId in mapping.items():
        mano_joints[manoId] = dhm_joints[dhmId]
    #import pdb; pdb.set_trace()
    return mano_joints

def ih262ManoS2HAND(ih26_joints):
    #import pdb; pdb.set_trace()
    mano_joints = np.zeros(ih26_joints.shape, dtype=np.float32) # init empty list

    # ih26Id, FreiId
    mapping = {0 :20,   1 :19,   2 :18,   3 :17,  # Thumb
               4 : 4,   5 : 3,   6 : 2,   7 : 1,  # Index
               8 : 8,   9 : 7,   10: 6,   11: 5,  # Middle
               12:16,   13:15,   14:14,   15:13,  # Ring
               16:12,   17:11,   18:10,   19: 9,  # Pinky
               20: 0}                             # Wrist

    for ih26Id, manoId in mapping.items():
        mano_joints[manoId] = ih26_joints[ih26Id]
    #import pdb; pdb.set_trace()
    return mano_joints
