...
def rot_pose_beta_to_mesh(rots, poses, betas, diff_v=None, mano_file = None):
    ...
    MANO_file = mano_file if mano_file is not None else os.path.join('assets/mano/models/MANO_RIGHT.pkl')
    ...
    mesh_num = dd['v_template'].shape[0]
    ...
    poses = torch.cat((root_rot.repeat(batch_size,1).view(batch_size,1,3),poses),1) # [b,21,3]
