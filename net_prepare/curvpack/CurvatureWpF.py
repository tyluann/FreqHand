...
def GetVertexNormalsExtra(vertices,faces,FaceNormals,e0,e1,e2):
    ...
    wfv1=FaceNormals/(L2[:,1]*L2[:,2] + 1e-12)[:,np.newaxis]
    wfv2=FaceNormals/(L2[:,2]*L2[:,0] + 1e-12)[:,np.newaxis]
    wfv3=FaceNormals/(L2[:,0]*L2[:,1] + 1e-12)[:,np.newaxis]
    ...
    Acorner=(0.5*Af/(ew[:,0]+ew[:,1]+ew[:,2] + 1e-12))[:,np.newaxis]*np.c_[ew[:,1]+ew[:,2], ew[:,2]+ew[:,0], ew[:,0]+ew[:,1]]

    #Change the area to barycentric area for obtuse triangles
    for i,f in enumerate(faces):
        if ew[i,0]<=0:
            Acorner[i,2]=-0.25*L2[i,1]*Af[i]/(sum(e0[i]*e1[i]) + 1e-12)
            Acorner[i,1]=-0.25*L2[i,2]*Af[i]/(sum(e0[i]*e2[i]) + 1e-12)
            Acorner[i,0]=Af[i]-Acorner[i,1]-Acorner[i,2]
        elif ew[i,1]<=0:
            Acorner[i,2]=-0.25*L2[i,0]*Af[i]/(sum(e1[i]*e0[i]) + 1e-12)
            Acorner[i,0]=-0.25*L2[i,2]*Af[i]/(sum(e1[i]*e2[i]) + 1e-12)
            Acorner[i,1]=Af[i]-Acorner[i,0]-Acorner[i,2]
        elif ew[i,2]<=0:
            Acorner[i,0]=-0.25*L2[i,1]*Af[i]/(sum(e2[i]*e1[i]) + 1e-12)
            Acorner[i,1]=-0.25*L2[i,0]*Af[i]/(sum(e2[i]*e0[i]) + 1e-12)
